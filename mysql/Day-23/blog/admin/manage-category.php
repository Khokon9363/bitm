<?php 
  ...
  $queryResult='';
  $queryResult=$login->viewcategory();
  ...
  if (isset($_GET['delete'])) {
           ...
           $id=$_GET['id'];
           $login=new Login();
           $delmassage=$login->deleteCategory($id);
  }
 ...
 <?php while ($login=mysqli_fetch_assoc($queryResult)) { ?>
  <tbody> 
    <tr> 
      ...
      <td><?php echo $login['description']; ?> </td> 
      <td><?php echo $login['status']; ?> </td>
      <td>
        <a href="edit-category.php?id=<?php echo $login['id']; ?>php">Edit</a>
